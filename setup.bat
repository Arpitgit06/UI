...
echo Downloading local LLMs (Qwen2.5-Coder-7B, Qwen2-VL-7B, Qwen2.5-VL-3B) right now so the tool works 100%% offline forever...
echo This will download approximately 36GB of weights. Please be patient depending on your internet speed...
"%VENV_PYTHON%" -c "from app.config import settings; import os, shutil; os.environ['HF_HOME'] = str(settings.models_cache_dir); os.environ['HF_HUB_DISABLE_SYMLINKS_WARNING'] = '1'; from huggingface_hub import snapshot_download; from transformers import pipeline; from ultralytics.utils.downloads import attempt_download_asset; print('Downloading Depth-Anything-V2 weights into models_cache...'); pipeline(task='depth-estimation', model=settings.depth_model_name); print('Downloading Qwen2.5-Coder-7B-Instruct...'); snapshot_download(repo_id='Qwen/Qwen2.5-Coder-7B-Instruct', max_workers=1); print('Downloading Qwen2-VL-7B-Instruct (verifier)...'); snapshot_download(repo_id='Qwen/Qwen2-VL-7B-Instruct', max_workers=1); print('Downloading Qwen2.5-VL-3B-Instruct (detector)...'); snapshot_download(repo_id='Qwen/Qwen2.5-VL-3B-Instruct', max_workers=1); print('Downloading YOLO models...'); [shutil.move(str(attempt_download_asset('yolov8n.pt')), settings.yolo_macro_weights_path) for _ in [0] if not os.path.exists(settings.yolo_macro_weights_path)]; [shutil.move(str(attempt_download_asset('yolov10n.pt')), settings.yolo_micro_weights_path) for _ in [0] if not os.path.exists(settings.yolo_micro_weights_path)]; print('Model pre-caching complete!')"
if %errorlevel% neq 0 (
