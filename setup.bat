...
echo Downloading local 7B LLM (Qwen2.5-Coder-7B-Instruct) and Vision models right now so the tool works 100%% offline forever...
echo This may take several minutes depending on internet speed...
"%VENV_PYTHON%" -c "from app.config import settings; import os; os.environ['HF_HOME'] = str(settings.models_cache_dir); from huggingface_hub import snapshot_download; from transformers import pipeline; print('Downloading Depth-Anything-V2 weights into models_cache...'); pipeline(task='depth-estimation', model=settings.depth_model_name); print('Downloading Qwen/Qwen2.5-Coder-7B-Instruct weights into models_cache...'); snapshot_download(repo_id='Qwen/Qwen2.5-Coder-7B-Instruct'); print('Model pre-caching complete!')"
if %errorlevel% neq 0 (
